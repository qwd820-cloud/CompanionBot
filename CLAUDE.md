...
## 开发流程 — 规划者 / 执行者 / 评估者 三角色闭环

每个功能开发或 bug 修复必须经过三个阶段，由三个独立角色驱动：

### 1. 规划者 (Planner)
- **何时触发**: 收到新需求、发现 bug、或评估者打回时
- **输出**: 明确的实施计划，包括：影响范围分析、修改文件列表、风险评估、验收标准
- **工具**: 使用 Claude Code Plan 模式 (`/plan`) 输出结构化计划
- **原则**: 不写代码，只做分析和设计。宁可多花时间规划，不要边写边改
...
- **约束**:
  - 严格按照计划执行，不擅自扩大范围
  - 每个文件修改后自动触发 ruff lint 检查（通过 hook）
  - 不添加计划外的功能、重构、注释
- **原则**: 最小变更，只做计划中的事

### 3. 评估者 (Evaluator)
- **何时触发**: 执行者完成代码变更后
- **执行**: `./scripts/evaluate.sh` 或按需选择子项
- **评估维度**:
  - **代码质量**: ruff lint + format 检查
  - **测试通过**: pytest 全部通过
  - **架构合规**: 层级依赖方向、模块完整性
  - **安全检查**: 无硬编码密钥、无危险模式
  - **运行时健康**: 服务可用性（如果在运行）
- **输出**: 通过/不通过 + 具体问题清单
- **不通过时**: 问题清单回到规划者，重新规划修复方案

### 闭环流程
```
需求 → [规划者] → 计划 → 用户确认 → [执行者] → 代码 → [评估者] → 通过? → 完成
                    ↑                                              ↓ (不通过)
                    └──────────── 问题清单 ←────────────────────────┘
```
